...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        total_success = spacex_df[spacex_df['class'] == 1].groupby('Launch Site')['class'].count().reset_index()
        fig = px.pie(
            total_success,
            values='class',
            names='Launch Site',
...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        filtered_data = site_data[(site_data['Payload Mass (kg)'] >= payload_range[0]) & 
                                  (site_data['Payload Mass (kg)'] <= payload_range[1])]
        
        fig = px.scatter(filtered_data, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title=f'Payload vs Mission Outcome at {selected_site}',
                         labels={'class': 'Mission Outcome', 'Payload Mass (kg)': 'Payload Mass (kg)'}
                        )
        return fig
